...
import stringdiff, codecs, collections, multiprocessing, os, sys, random
...
def convert(word):
	...
	if maxDist >= threshold*len(word):
		...
		return word, maxWords, maxDist/float(len(word))
...
def convertMulti(corpusid, outid, nbThreads, filter=False, randomSelect=False, dictid=None):
	...
	if nbThreads > 1:
		...
		results = [convert(word) for word in rlWords]
...
if __name__ == "__main__":
	...
	os.system("head -n 89886 {}.ru2.train.txt > {}.temp.txt".format(id, id))
